...
async function getProblemDetails(req, res, next) {
  ...
  }).populate(`templates.${programmingLanguage}`);
  const testCases = problem.input_testcases;
  const solutionCodeSnippet = problem.templates.get(`${programmingLanguage}`)
    .solution_code_snippet;
  const codeContainer = problem.templates.get(`${programmingLanguage}`)
    .executable_code_container;
  const userCode = codeContainer.replace("INSERT_SOLVER", userCodeSnippet);
  const solutionCode = codeContainer.replace(
    "INSERT_SOLVER",
    solutionCodeSnippet
  );
  ...
  req.problemBO = {
    dockerDetails,
    testCases,
    snippets: { userCode, solutionCode, checkerCode },
  };
  next();
}
router.post("/execute/:problemName", getProblemDetails, async (req, res) => {
  ...
  const checkerDockerDetails = {
    dockerImageName: checkerImageName,
