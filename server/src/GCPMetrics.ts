import { globalStats, MeasureUnit, AggregationType, Measure } from '@opencensus/core'
...
export function recordMetric(measure: Measure, value: number) {
    globalStats.record([
        {
            measure: measure,
            value: value
...
export function initializeGCPMetrics() {
    ...
    try {
        const googleExporter = new StackdriverStatsExporter({ projectId: projectId, period: EXPORT_INTERVAL * 1000 })
