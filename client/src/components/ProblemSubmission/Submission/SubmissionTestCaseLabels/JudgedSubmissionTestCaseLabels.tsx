...
import AcceptedTabLabel from './AcceptedTabLabel';
import WrongAnswerTabLabel from './WrongAnswerTabLabel';
  ...
  return (
    <div>
      {judgedTestCases.map((_, index) => {
        ...
        if (_.status === VerdictStatus.ACCEPTED) return <AcceptedTabLabel labelText={labelString} index={index.toString()} />;
        else {
          return <WrongAnswerTabLabel labelText={labelString} index={index.toString()} />;
        }
      })}
    </div>
  );
