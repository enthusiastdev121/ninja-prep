...
import { INTERNAL_ERROR, SUBMIT_CODE, SUBMIT_YOUR_CODE } from './TestCaseAreaStringIds'
            ...
            {({ submissionOutput }) => {
                if (submissionOutput) {
                    switch (submissionOutput.verdict) {
                        ...
                        case 'Wrong Answer': {
                            const countFailed = submissionOutput.judged_test_cases.filter((testCase: any) => {
                                return testCase.status === 'ACCEPTED'
                            }).length
                            return (
                                <p className="h4 font-weight-bold d-inline text-danger">
                                    {countFailed}/{submissionOutput.judged_test_cases.length} Test Cases Passed
                                </p>
                            )
                        }
                        default: {
                            return <p className="h4 font-weight-bold d-inline text-danger"> {submissionOutput.verdict} </p>
                        }
                    }
                } else {
                    return <p className="h5 font-weight-bold d-inline text-danger"> {INTERNAL_ERROR} </p>
                }
            }}
        </UserSubmissionContext.Consumer>
    )
                    ...
                    <Col>
                        <div className="float-right">
                            <Button
                                ...
                                type="submit"
                                onClick={(e) =>
                                    handleSubmit(e, codeSnippet, mode, match.params.id, setSubmissionStatus, setUserSubmission)
                                }
                            >
                                <PlayArrowIcon />
                                <b>{SUBMIT_CODE}</b>
